...
public class ActiveProvider {
    public static void main(String[] args) throws Exception{
        ...
        while (true){
            Thread.sleep(1000);
            sendMsg(session,producer,"你好ActiveMQ,这是消息"+i);
            i++;
            if (i>20){
                break;
            }
    ...
    //发送消息
    private static void sendMsg(Session session, MessageProducer producer, String msg) throws JMSException {
        //构造一条文本消息
        TextMessage message = session.createTextMessage("消息内容为："+ msg);
        //消息生产者发送消息
        producer.send(message);
    }
}
